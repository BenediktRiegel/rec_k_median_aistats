...
void saveResult(string path, kMSolution S, vector<int> itr, double duration, fixedDouble lam, int k) {
	string filepath = path + "EvaluationResults/kUFLLocalSearchRandomSampling/k=" + to_string(k) + "_lam=" + stringValue(lam) + ".txt";
	cout << path << endl;
	string result;
	result += "S=";
...
void evalTwitter(string path, int num_runs, int sampleAmount) {
    ...
    for (int run = 0; run < num_runs; run++) {
        ...
        for (int k : ks) {
            for (double lam : lams) {
                ...
                pair<kMSolution, vector<int>> SAndItr = recsolve(&C, &F, &dFtoF, &dAtoC, lam, k, sampleAmount);
